...
const { extractFields } = require("../services/parser");
...
async function processJob(job) {
  ...
  try {
    ...
    let extracted;
    let ocrText = null; // Store OCR text for fallback parser
    ...
    try {
      ...
      try {
        ...
        // Fallback to OCR if Gemini Vision fails
        console.log("  → Gemini failed, falling back to OCR (EasyOCR)");
        ...
        ocrText = await extractText(job.filePath);
        ...
        try {
          ...
          // Gemini text analysis also failed (likely quota exceeded)
          console.error("  → Gemini text analysis failed:", geminiTextError.message);
          
          // Final fallback: Use pattern-based parser (NO API calls)
          console.log("  → Using pattern-based parser (no API)...");
          extracted = extractFields(ocrText);
          console.log("  → Pattern parser complete");
        }
      }
    } catch (ocrError) {
      // OCR completely failed - this is a rare case
